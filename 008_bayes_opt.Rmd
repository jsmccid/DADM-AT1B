...
# disco_team_mod <- 0.95 # % of potential teams
# discoteam_last_costs <- 0.05 # minus percentage of last years costs from discovery team budget (of starting cap for first year)
# 
# # legal modifier
# prob_of_choosing_resolution <- 0.05 # Probability of chosing to try and resolve
# prob_of_choosing_resolution_found_claim <- 0.05 # probability if 2nd claim is found (independent of above)
...
```{r}
bobounds <- list(
  discoteam_last_costs = c(0,1),
  prob_of_choosing_resolution = c(0,1),
  prob_of_choosing_resolution_found_claim = c(0,1),
  msp1 = c(0,1),
  msp2 = c(0,1),
  msp3 = c(0,1),
  msp4 = c(0,1),
...
```{r}
fullmonte <- function(discoteam_last_costs, prob_of_choosing_resolution, prob_of_choosing_resolution_found_claim, msp1,msp2,msp3,msp4,msp5, ghm1,ghm2,ghm3,ghm4,ghm5, dts1,dts2,dts3,dts4,dts5, dto1,dto2,dto3,dto4,dto5, dof1,dof2,dof3,dof4,dof5, rof1,rof2,rof3,rof4,rof5) {
  ...
  minestanddown_prob <- c(msp1,msp2,msp3,msp4,msp5)
  gold_hold_mod <- c(ghm1,ghm2,ghm3,ghm4,ghm5) # percentage capital spent on buying gold
  dtoswitch <- c(dts1,dts2,dts3,dts4,dts5)
  discoteam_overide <- c(dto1,dto2,dto3,dto4,dto5)
  disco1to5 <- c(dof1,dof2,dof3,dof4,dof5)
  reso1to5 <- c(rof1,rof2,rof3,rof4,rof5)
  
  #static

  # discovery teams modifier
disco_team_mod <- 1 # % of potential teams
...
out <- quantile(ecdf(y5_end_cap), c(0.05, 0.5, 0.95), type = 7)

# qcalc <- out[[1]] - (log(out[[3]]-out[[2]])*1e5)
qcalc <- out[[1]] + out[[2]] + out[[3]]

return(list(Score = qcalc))
}
```
...
opt <- bayesOpt(fullmonte, bounds = bobounds, initPoints = 100, iters.n = 4, iters.k = 2, acq = "ucb", kappa = 2.576, eps = 0, verbose = TRUE, plotProgress = TRUE, parallel = TRUE)
```
