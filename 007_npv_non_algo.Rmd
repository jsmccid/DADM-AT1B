...
disco_team_mod <- 0.1 # % of potential teams
discoteam_last_costs <- 0.0 # minus percentage of last years costs from discovery team budget (of starting cap for first year)

# legal modifier
prob_of_choosing_resolution <- 0.9 # Probability of chosing to try and resolve
prob_of_choosing_resolution_found_claim <- 0.9 # probability if 2nd claim is found (independent of above)
...
# RBA cash rate - Savings rate
# 0.0125

# Gold price increase rate based on estimate
# 0.05
...
calcteam <- floor(runif(num_sim, minteam, maxteam)*disco_team_mod)
...
goresolve <- rbinom(num_sim, 1, prob_of_choosing_resolution)
...
cap_y2_s1 <- y1_end_cap + emine_gross
...
cap_y3_s1 <- y2_end_cap + emine_gross
...
cap_y4_s1 <- y3_end_cap + emine_gross
...
cap_y5_s1 <- y4_end_cap + emine_gross
